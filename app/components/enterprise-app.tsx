    ...
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <div className="app-main">
        ...
        <main className="app-content">
          ...
          {currentView === "billing" && canUse("billing") && (activeProjectId ? <BillingView language={language} notify={notify} projectId={activeProjectId} canManage={canManage("billing")} canManagePayments={canManage("billing") && canManage("finance")} canManageTaxes={["Admin", "Finance"].includes(user.role) && canManage("billing") && canManage("finance")} canManageValidity={["Admin", "Finance"].includes(user.role) && canManage("billing")} onOpenProject={() => navigate("project")} /> : <ProjectContextEmpty language={language} onDashboard={() => navigate("dashboard")} />)}
          {currentView === "procurement" && canUse("procurement") && (activeProjectId || ["Admin", "Finance"].includes(user.role) ? <ProcurementView language={language} notify={notify} projectId={activeProjectId || undefined} canManage={canManage("procurement")} canManageCommercial={canManage("boq") && canManage("billing")} canManageVendors={["Admin", "Finance"].includes(user.role) && canManage("procurement")} canManagePayments={canManage("procurement") && canManage("finance")} canViewTaxes={canUse("finance")} userRole={user.role} /> : <ProjectContextEmpty language={language} onDashboard={() => navigate("dashboard")} />)}
