...
const guides = {
  id: [
    ...
    { title: "Database Item dan harga jual", icon: FileSpreadsheet, body: "Admin/Finance menyusun item menurut peran BoQ, kategori produk, merek, dan model. Kategori perangkat utama adalah Networking, CCTV, IP PABX, dan Smart Home Device. Harga 1 dan Harga 2 dihitung dari harga pokok serta margin kategori; item tertentu dapat memakai margin khusus. PM/Engineer memilih item dan tier harga dari katalog saat menyusun BoQ." },
    { title: "Quotation, PPN klien, invoice, dan pembayaran", icon: ReceiptText, body: "Quotation mengambil nilai dari BoQ proyek aktif. Pajak tambah seperti PPN dibebankan kepada klien di atas nilai dasar, tidak menjadi biaya proyek PerumNet, dan dicatat sebagai utang pajak. Invoice mewarisi snapshot pajak Quotation. Konfirmasi pembayaran hanya mencatat kas aktual yang diterima." },
    ...
    { title: "Belanja proyek dan bukti nota", icon: ReceiptText, body: "PM/Engineer mencatat belanja hanya pada proyek yang dapat diakses, lalu mengunggah JPG, PNG, WebP, atau PDF. Finance memeriksa nota, kategori, sumber dana, dan potensi duplikasi sebelum menyetujui. Rekening perusahaan mencatat kas keluar; uang muka hanya mengurangi saldo; uang pribadi menjadi utang reimbursement." },
    { title: "Uang muka dan reimbursement", icon: ShieldCheck, body: "Finance mencatat pencairan uang muka sebagai kas keluar satu kali. Nota yang menggunakan uang muka tidak membuat kas keluar kedua. Reimbursement dapat dibayar sebagian. Admin melakukan Void melalui reversal, dan transaksi yang telah direkonsiliasi harus dilepas terlebih dahulu." },
    { title: "Pajak opsional dan snapshot", icon: ReceiptText, body: "Admin mengaktifkan modul pajak dan menentukan kode, cakupan, efek tambah/potong, tarif, serta perlakuan akuntansi. Admin/Finance menyalakan switch Gunakan Pajak dan memilih aturan pada Quotation Draft. Snapshot terkunci saat Accepted dan diwariskan ke Invoice; tarif master sesudahnya tidak mengubah histori." },
  ...
  en: [
    ...
    { title: "Item Database and selling prices", icon: FileSpreadsheet, body: "Admin/Finance organizes items by BoQ role, product category, brand, and model. The main device categories are Networking, CCTV, IP PABX, and Smart Home Device. Price 1 and Price 2 are calculated from cost and category margins, with optional item-level margins. PMs/Engineers select catalog items and a price tier when preparing a BoQ." },
    { title: "Quotations, client VAT, invoices, and payments", icon: ReceiptText, body: "A quotation uses the active project’s BoQ. Added tax such as VAT is charged to the client above the taxable base, is not a PerumNet project cost, and is recorded as tax payable. The Invoice inherits the Quotation tax snapshot. Payment confirmation records only actual cash received." },
    ...
    { title: "Project expenses and receipts", icon: ReceiptText, body: "PMs/Engineers record purchases only for accessible projects and upload JPG, PNG, WebP, or PDF evidence. Finance reviews the receipt, category, funding source, and duplicate warnings before approval. Company funds post cash out, advances only reduce their balance, and employee-paid costs become reimbursement payables." },
    { title: "Advances and reimbursements", icon: ShieldCheck, body: "Finance records an advance disbursement as a single cash outflow. Receipts allocated to that advance never post cash a second time. Reimbursements support partial payment. Admin voids through reversal, and reconciled transactions must be detached first." },
    { title: "Optional tax and snapshots", icon: ReceiptText, body: "An Admin enables the tax module and defines the code, scope, add/withhold effect, rate, and accounting treatment. Admin/Finance enables Use Tax and selects rules on a Draft Quotation. The snapshot locks at Accepted and passes to its Invoice, so later master-rate changes never rewrite history." },
