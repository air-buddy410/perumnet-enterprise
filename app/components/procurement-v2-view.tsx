            ...
            {orders.map((order) => (
              <article className="procurement-order-card" key={order.id}>
                ...
                {!order.legacy && order.approvalStatus === "Approved" && order.workflowStatus !== "Void" && order.availableToPay === 0 && order.outstanding > 0 && (
                  <div className="order-payment-hold">
                    <PackageCheck size={15} />
                    <span>{order.documentType === "PO"
                      ? (id ? "Menunggu penerimaan barang sebelum termin berikutnya dapat dibayar." : "Awaiting goods receipt before the next payment term can be paid.")
