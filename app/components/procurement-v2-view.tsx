...
export function ProcurementViewV2({
  ...
  return (
    <div className="page-stack procurement-v2" data-testid="procurement-view">
      ...
      {showVendor && <div className="modal-backdrop" onMouseDown={() => setShowVendor(false)}><section className="modal-card wide" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">{id ? "MASTER VENDOR" : "VENDOR MASTER"}</span><h2>{editingVendor ? (id ? "Edit vendor" : "Edit vendor") : (id ? "Vendor baru" : "New vendor")}</h2></div><button className="icon-button" type="button" onClick={() => setShowVendor(false)}><X size={18} /></button></div><form className="form-grid" onSubmit={saveVendor}>
      ...
      {showOrder && <div className="modal-backdrop" onMouseDown={() => setShowOrder(false)}><section className="modal-card extra-wide" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">{editingOrder ? (id ? "EDIT DRAFT KOMITMEN" : "EDIT COMMITMENT DRAFT") : (id ? "KOMITMEN VENDOR BARU" : "NEW VENDOR COMMITMENT")}</span><h2>{documentType === "PO" ? "Purchase Order" : id ? "Surat Perintah Kerja" : "Work Order"}</h2></div><button className="icon-button" type="button" onClick={() => setShowOrder(false)}><X size={18} /></button></div><form className="form-grid" onSubmit={saveOrder}>
