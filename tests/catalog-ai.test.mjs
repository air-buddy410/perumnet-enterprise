...
function responsesPayload(recommendation) {
  return {
...
async function startApp(extraEnv) {
  ...
  databasePath = `/tmp/perumnet-catalog-ai-${process.pid}-${Date.now()}.db`;
  uploadDirectory = `/tmp/perumnet-catalog-ai-uploads-${process.pid}-${Date.now()}`;
  cookie = "";
  server = spawn(
    process.execPath,
...
test(
  "catalog AI analyze fails closed without an OpenAI key and burns no quota",
  { timeout: 120_000 },
  async () => {
    await startApp({ OPENAI_API_KEY: "" });
    await login();
    ...
    assert.equal(payload.error.code, "OPENAI_NOT_CONFIGURED");
...
test(
  "catalog AI runs asynchronously with polling, failure handling, sweeper, and approval",
  { timeout: 240_000 },
  async () => {
    ...
    mockServer = createHttpServer((incoming, response) => {
      ...
      incoming.on("end", () => {
        ...
        response.end(JSON.stringify(responsesPayload(recommendation)));
      });
    });
    ...
    // a. Happy path: 202 + Running, poll to Draft, recommendation and sources persisted.
    mockMode = "happy";
    ...
    assert.equal(draftRun.model, "gpt-5-mini-mock");
    assert.equal(draftRun.confidence, 85);
    assert.equal(draftRun.recommendation.sku, "RG-RAP2260-AI");
    ...
    assert.equal(draftRun.sources.length, 1);
    assert.equal(draftRun.sources[0].url, "https://example.com/product");
    assert.equal(draftRun.sources[0].title, "Contoh Produk");
    const mine = await json("/api/catalog/ai?mine=1");
    assert.ok(mine.some((run) => run.id === startedRun.id));

    // b. Structurally invalid model output marks the run Failed with a descriptive message.
    mockMode = "bad";
