...
export function ProjectExpenseView({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (canReview) {
        const accounts = await api<BankAccount[]>("/api/bank-accounts");
        setBankAccounts(accounts.filter((item) => item.status === "Aktif"));
        setReviewBankId((current) => current || accounts.find((item) => item.status === "Aktif")?.id || "");
      }
    } catch (error) {
  ...
  }, [canReview, language, notify, projectId]);
  ...
  const loadEligible = useCallback(async (targetProjectId: string) => {
    ...
    try {
      const people = await api<EligibleUser[]>(`/api/project-advances/eligible-users?projectId=${encodeURIComponent(targetProjectId)}`);
      setEligibleUsers(people);
      setAdvanceRecipientId((current) => people.some((person) => person.id === current) ? current : people[0]?.id ?? "");
    } catch (error) { notify(messageOf(error, language)); }
  }, [language, notify]);
      ...
      {formOpen && canCreate && <section className="panel expense-capture-panel">
        ...
        <form className="expense-quick-form" onSubmit={submitExpense}>
          <div className="form-grid">
            <label className="field full"><span>{id ? "Proyek" : "Project"}</span><select required value={expenseProjectId} onChange={(event) => setExpenseProjectId(event.target.value)} disabled={Boolean(projectId)}><option value="">{id ? "Pilih proyek" : "Select project"}</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.code} · {project.name}</option>)}</select></label>
            ...
            <label className="field full"><span>{id ? "Sumber dana" : "Funding source"}</span><select value={fundingSource} onChange={(event) => setFundingSource(event.target.value as ProjectExpense["fundingSource"])}><option value="CompanyAccount">{fundingLabel(language, "CompanyAccount")}</option><option value="ProjectAdvance">{fundingLabel(language, "ProjectAdvance")}</option><option value="EmployeePaid">{fundingLabel(language, "EmployeePaid")}</option></select></label>
